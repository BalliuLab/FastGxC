...
#' @param four_level - boolean value (T or F) that signifies whether to use the 4-level hierarchy with TreeBH (testing for global eQTL across shared and specific components). Default is FALSE for 3-level hierarchy. Note: cannot be used with unified_hierarchy = TRUE.
#' @param unified_hierarchy - boolean value (T or F) that signifies whether to use unified hierarchy where shared and specific components are tested at the same level (Level 1: Gene, Level 2: Shared + Contexts, Level 3: SNPs). When TRUE, outputs combined_eGenes.txt instead of separate shared/specific files. Default is FALSE. Note: cannot be used with four_level = TRUE.
...
#' @return Outputs different files depending on the mode:
#' \itemize{
...
treeQTL_step <- function(data_dir, snps_location_file_name, gene_location_file_name, context_names, out_dir, cisDist = 1e6, fdr_thresh = 0.05, four_level = F, unified_hierarchy = F, qtl_type = "cis", treeBH_method = "cpp", treeBH_test = "simes") {
  ...
  # Validate that four_level and unified_hierarchy are not both TRUE
  if (four_level && unified_hierarchy) {
    stop("unified_hierarchy is not supported with four_level = TRUE (TreeBH). Please set one of them to FALSE.")
  }
  ...
  shared_n_tests_per_gene <- get_n_tests_per_gene(
    snp_map = snpspos %>% dplyr::select(snpid, chr, pos), gene_map = genepos %>% dplyr::select(geneid, chr, s1, s2),
  ...
  if (unified_hierarchy) {
    # Use unified hierarchy: Level 1 (Gene) -> Level 2 (Shared + Contexts) -> Level 3 (SNPs)
    print("Using unified hierarchy where shared and specific components are at the same level")
    combined_eGenes <- get_eGenes_combined(
    ...
    # Use original separate hierarchies for shared and specific
