...
get_eGenes_multi_tissue_mod <- function(m_eqtl_out_dir, treeQTL_dir, tissue_names, level1 = 0.05, level2 = 0.05, level3 = 0.05, exp_suffix, four_level = FALSE, qtl_type = "cis", shared_n_tests_per_gene) {
...
#
#    print(paste("Computing summary statistics for tissue ", cur_tissue_name, sep = ""))
#    #n_SNPs_per_gene_this_tissue <- data.frame(fread(input = n_SNPs_per_gene_outfiles[i], header = F), stringsAsFactors = F,check.names = FALSE)
#    #colnames(n_SNPs_per_gene_this_tissue)=c("family","n_tests")
...
get_pvals_and_fam_p_mod <- function(genes_by_tissue, snps_by_tissue, m_eqtl_out_dir, tissue_names, exp_suffix) {
  ...
  for (i in 1:n_tissue) {
    ...
    cur_data_table <- data.frame(cur_data %>% mutate(pair_names = paste(SNP, gene, sep = "*")) %>%
      select(pair_names, SNP, gene, p.value), check.names = FALSE, stringsAsFactors = F)
