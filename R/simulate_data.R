...
simulate_data <- function(data_dir, N = 300, n_genes = 100, n_snps_per_gene = 1000,
  ...
  if (requireNamespace("data.table", quietly = TRUE)) {
    data.table::fwrite(snp_out,
      file = file.path(data_dir, "SNPs.txt"),
  ...
  fwrite(
    x = snp_loc, file = paste0(data_dir, "snpsloc.txt"), quote = FALSE, sep = "\t", row.names = FALSE,
    col.names = TRUE
  )
  fwrite(
    x = gene_loc, file = paste0(data_dir, "geneloc.txt"), quote = FALSE, sep = "\t", row.names = FALSE,
    col.names = TRUE
  )
  ...
  for (i in 1:n_genes) {
    ...
    data_mat <- melt(
      data = data.table(Y, keep.rownames = TRUE),
      id.vars = "rn"
    )
  ...
  fwrite(
    x = exp_mat_missing[, -c(2:3)], file = paste0(data_dir, "expression.txt"), quote = FALSE,
    sep = "\t", row.names = FALSE, col.names = TRUE
  )
