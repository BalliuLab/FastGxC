...
if(1){
  ...
  #### Data preprocess 
  if(1){
    sim_res = fread(file = paste0("../Input_Files/Figure2_Simulation/simulation_results_typeIErrorPower_2025.txt"), sep = "\t", data.table = F) %>%
      mutate(nC = paste0("#Contexts:", nC),
             ...
             method=factor(x = method, levels = c("CxC", "CxC-unadjasted", "LM-GxC", "LMM-GxC", "FastGxC")) #, "METASOFT", "Meta-Tissue"
      )
...
if(1){
  ...
  ggsave(filename = 'Fig07_final.pdf', plot = fig7, width = 15, height = 15, limitsize = F)
